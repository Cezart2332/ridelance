...
/**
 * Formularul de cerere (spec §17). Un singur dialog pentru toate intrările: lista de mașini,
 * dashboardul și pagina de detaliu.
 *
          ...
          <Box sx={{
            ...
            <Box sx={{
              ...
              {cover ? (
                ...
                <DirectionsCarFilledRoundedIcon sx={{ color: DASHBOARD_TOKENS.textSubtle }} />
              )}
            </Box>

            <Box sx={{ minWidth: 0, flex: 1 }}>
              <Typography noWrap sx={{ fontWeight: 800, color: DASHBOARD_TOKENS.ink }}>
                {car ? `${car.brand} ${car.model} ${car.year}` : 'Alege un vehicul din listă'}
              </Typography>
              {car && (
                <Typography sx={{ fontSize: '0.85rem', color: DASHBOARD_TOKENS.textSubtle }}>
                  {car.pricePerWeek.toLocaleString('ro-RO')} lei / săptămână
                </Typography>
              )}
            </Box>

            <IconButton onClick={handleClose} aria-label="Închide" sx={{ color: alpha(DASHBOARD_TOKENS.ink, 0.4) }}>
              <CloseRoundedIcon />
            </IconButton>
          </Box>

          <Box sx={{ p: 3, overflowY: 'auto', maxHeight: 'calc(90vh - 96px)' }}>
            {!submitted ? (
                ...
                <Stack spacing={2.5} component="div">
                  ...
                  >
                    {submitting
                      ...
                      : waitlist ? 'Anunță-mă când e liberă' : 'Trimite cererea'}
                  </Button>

                  <Typography sx={{ fontSize: '0.78rem', color: DASHBOARD_TOKENS.textSubtle, textAlign: 'center' }} component="p">
