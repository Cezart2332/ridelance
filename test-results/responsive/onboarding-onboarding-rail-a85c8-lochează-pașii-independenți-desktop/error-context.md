...
- Name: onboarding.spec.ts >> onboarding rail >> un pas în verificare nu blochează pașii independenți
- Location: tests/responsive/onboarding.spec.ts:207:3
...
Locator:  getByRole('button', { name: /Fiscal, bancă & semnături/ }).first()
Expected: enabled
Received: disabled
Timeout:  15000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /Fiscal, bancă & semnături/ }).first()
    27 × locator resolved to <button disabled tabindex="-1" type="button" aria-label="Fiscal, bancă & semnături — Blocat" class="MuiButtonBase-root Mui-disabled css-t9pyze-MuiButtonBase-root">…</button>
